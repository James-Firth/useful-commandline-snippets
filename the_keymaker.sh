...
if [[ $first_key == true ]]; then
	...
	echo "#Generated by the_keymaker o——m v2.6" > $ssh_config_path
...
if [ $keygen_status -eq 0 ]; then
	...
	# TODO: the port is a little ugly, but I think it's nicer than an IF with practically the same info? Look at improving it
	echo "
Host $shortname
	User $remoteuser
	HostName $hostname$port_line
